    ...
    def _drain_queue(self) -> None:
        while True:
            ...
            if typ == "forecast_progress":
                ...
                train_reward_per_step = data.get("train_reward_per_step", data["train_reward"])
                eval_per_step = data.get("eval_reward_per_step")
                eval_part = "" if eval_per_step is None else f" eval/step={eval_per_step:.4f}"
                self.dqn_status.configure(
                    text=(
                        ...
                        f"steps={data.get('episode_steps', 0)} | "
                        f"reward/step={train_reward_per_step:.4f}{eval_part} | ETA {format_eta(data['eta_seconds'])}"
                    )
                )
